...
Finally, this is a work in progress. It has an active #link("https://github.com/T-U-R-A/tura-coding-book")[GitHub repository] and contributions from the community are warmly welcomed. With collective effort, this book can become more comprehensive, refined, and enjoyable for readers around the world.
...
=== Binary Indexed Tree <fenw> //chap2
...
There are many other questions where you can use a greedy approach, and you'll understand how to use them by solving such questions#footnote[See @deadlines].
...
We can use a stack#footnote[See @stack] of pairs (value, index) that stores the previously seen values that are less than the current value `x` in ascending order. This is achieved by first popping all elements greater than `x` and then either outputting the index at the top of the stack (`s.top.second`) or if the stack is empty, outputting zero. Finally push `x` into the stack.
